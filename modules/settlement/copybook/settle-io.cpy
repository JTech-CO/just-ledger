      ...
      * 1. DIRECTION 은 'D'/'C' 만 유효. 생성기(Go/JS)는 그 외를 거절하고,
      *    COBOL 도 만나면 ERROR STATUS 8 로 즉시 중단한다 (조용한 부호
      *    추정 금지 — JS 참조와 부호가 갈라지면 INV-7 위반).
      * 2. 고유 계정 수 최대 5000 (ACC-TABLE OCCURS 상한). 생성기가 실행
      *    전에 거절하고, COBOL 도 초과 시 ERROR STATUS 8 로 중단한다.
      * 3. 환산값·계정 누적 잔액은 S9(18) 범위. 생성기는 entry 단위
      *    환산 상한(floor(amount*num/den)+1 <= 10^18-1)을 보수적으로
      *    사전 검증한다. 누적 오버플로는 반올림 정본을 재구현하지 않고는
      *    실행 전 정확 판정이 불가하므로(CLAUDE.md 반올림 규칙),
      *    COBOL 이 ON SIZE ERROR 로 감지해 ERROR STATUS 8 로 중단한다.
      *    어느 경로에도 조용한 절삭은 없다.
      *****************************************************************
