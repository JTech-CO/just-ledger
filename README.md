...
**[▶ UI 데모 열기](https://jtech-co.github.io/just-ledger/)** · **[📲 앱으로 설치하기](https://jtech-co.github.io/just-ledger/app/)**

- **UI 데모** — 설치 없이 화면만 바로 봅니다. 모의 데이터가 채워져 있고 새로고침하면 초기화됩니다.
- **설치형 앱(PWA)** — 위 앱 링크를 열고 브라우저 주소창의 **설치** 아이콘(또는 메뉴 → "앱 설치")을 누르면 데스크톱·모바일에 앱으로 설치됩니다. 다운로드·서명 불필요. **오프라인**에서 동작하며 데이터는 **이 기기에만** 저장됩니다(내보내기로 백업).

두 버전 모두 **UI 계층**만 담습니다 — 복식부기(차변=대변)는 앱 안에서 검사하지만, 마감 정합성(COBOL)·자동 분류(Prolog)·다중 사용자는 서버 계층이라 여기서는 동작하지 않습니다. 전체 기능은 아래 셀프호스팅으로 띄웁니다.
