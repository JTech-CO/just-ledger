...
describe('잔액 표시: signed tone — 부호로 색, 통화기호 없음', () => {
  it('양수는 + 부호 + 초록(positive) 색', () => {
    const { container } = render(<Money minor="1234" tone="signed" signed />);
    const money = container.querySelector('[data-amount="1234"]');
    // debit 클래스(=--positive 초록)가 붙는다
    expect(money.className).toMatch(/debit/);
    expect(container.textContent).toContain('+');
  });

  it('음수는 − 부호 + 빨강(negative) 색', () => {
    const { container } = render(<Money minor="-1234" tone="signed" signed />);
    const money = container.querySelector('[data-amount="-1234"]');
    expect(money.className).toMatch(/credit/);
    expect(container.textContent).toContain('−');
  });

  it('0 은 색·부호 없음', () => {
    const { container } = render(<Money minor="0" tone="signed" signed />);
    const money = container.querySelector('[data-amount="0"]');
    expect(money.className).not.toMatch(/debit|credit/);
    expect(container.textContent).not.toContain('+');
    expect(container.textContent).not.toContain('−');
  });

  it('통화 없이 렌더하면 ₩ 기호가 없다', () => {
    const { container } = render(<Money minor="1000" tone="signed" signed />);
    expect(container.textContent).not.toContain('₩');
  });
});
