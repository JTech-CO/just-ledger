...
function genSettle(count, seed) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    // 반올림이 실제로 갈리는 값이 섞이도록 홀수·다양한 자릿수
    const amount = String(1 + Math.floor(rnd() * 9_999_999));
    entries.push({
...
function writeAmort() {
  ...
  for (const a of LOANS) {
    ...
    const payment = levelPayment(
      BigInt(a.principal), BigInt(a.rate_num), BigInt(a.rate_den), Number(a.periods),
    ).toString();
    inLines.push(amortInLine({ ...a, payment }));
    const sch = amortReference(a.principal, a.rate_num, a.rate_den, Number(a.periods));
